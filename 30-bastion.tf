...
module "bastion" {
  ...
  user_data                   = "${var.linux_userdata}${data.template_file.linux_bastion_userdata.rendered}"
...
module "bastion-win" {
  ...
  user_data                   = "<powershell>${var.windows_userdata}${data.template_file.bastion_domain_join_ps1.rendered}${data.template_file.bastion_r53_register_ps1.rendered}</powershell><persist>true</persist>"
  associate_public_ip_address = true
